...
- **보드**: 트렐로 방식 — 보드 > 리스트 > 카드, 드래그&드롭으로 이동. 담당자별 필터 지원(기본 전체).
...
- **첨부 모아보기**: 조직·보드별로 올린 파일과 링크를 한 곳에서 보고 파일명·업무명으로 검색, 연결된 업무로 바로 이동.
- **담당자 필터**: 칸반과 네 가지 보기 모두 담당자별로 걸러 볼 수 있습니다. **여러 명 동시 선택**(OR 조건)을 지원하고, `?assignee=a,b` 로 URL에 남아 보기를 바꿔도 유지·공유됩니다.
